...
*
* A modernized version of the provided jsSHA library / github.com/gtbu 8/2025
* @param {object} J - The global object, typically 'this' or 'window'.
*/

'use strict';
(function (J) {
    /**
     * Constructor for the jsSHA object.
     * @param {string} a The input string.
     * @param {string} b The input format (HEX, TEXT, B64, BYTES).
     * @param {string} c The encoding (UTF8, UTF16BE, UTF16LE).
     */
    function w(a, b, c) {
        let f = 0,
            g = [0],
            ...
            h = null;
        l = c || "UTF8";
        if ("UTF8" !== l && "UTF16BE" !== l && "UTF16LE" !== l) throw "encoding must be UTF8, UTF16BE, or UTF16LE";
        if ("HEX" === b) {
        ...
        this.getHash = function (a, b, c, l) {
            ...
            if ("SHA-384" === a)
                for (p = 0; p < c; p += 1) d = u(d, n, a), n = 384;
            else if ("SHA-512" === a)
                for (p = 0; p < c; p += 1) d = u(d, n, a), n = 512;
        ...
        this.getHMAC = function (a, b, c, h, m) {
            ...
            if ("SHA-384" === c) n = 128, r = 384;
            else if ("SHA-512" === c) n = 128, r = 512;
            else throw "Chosen SHA variant is not supported";
            if ("HEX" === b) d = y(a), p = d.binLen, d = d.value;
            else if ("TEXT" === b || "ASCII" === b) d = z(a, l), p = d.binLen, d = d.value;
            else if ("B64" === b) d = A(a), p = d.binLen, d = d.value;
            else if ("BYTES" === b) d = B(a), p = d.binLen, d = d.value;
            else throw "inputFormat must be HEX, TEXT, ASCII, B64, or BYTES";
            a = 8 * n;
            b = n / 4 - 1;
            if (n < p / 8) {
                for (d = u(d, p, c); d.length <= b;) d.push(0);
                d[b] &= 4294967040
            } else if (n > p / 8) {
                for (; d.length <= b;) d.push(0);
                d[b] &= 4294967040
            }
            for (n = 0; n <= b; n += 1) q[n] = d[n] ^ 909522486, H[n] = d[n] ^ 1549556828;
            c = u(H.concat(u(q.concat(g), a + f, c)), a + r, c);
            return h(c, G(m))
        }
    }
    ...
    function z(a, b) {
        ...
        if ("UTF8" === b)
            for (h = 0; h < a.length; h += 1)
                for (f = a.charCodeAt(h), g = [], 128 > f ? g.push(f) : 2048 > f ? (g.push(192 | f >>> 6), g.push(128 | f & 63)) : 55296 > f || 57344 <= f ? g.push(224 | f >>> 12, 128 | f >>> 6 & 63, 128 | f & 63) : (h += 1, f = 65536 + ((f & 1023) << 10 | a.charCodeAt(h) & 1023), g.push(240 | f >>> 18, 128 | f >>> 12 & 63, 128 | f >>> 6 & 63, 128 | f & 63)), s = 0; s < g.length; s += 1) {
                    for (m = l >>> 2; c.length <= m;) c.push(0);
                    c[m] |= g[s] << 24 - l % 4 * 8;
                    l += 1
                } else if ("UTF16BE" === b || "UTF16LE" === b)
            for (h = 0; h < a.length; h += 1) {
                f = a.charCodeAt(h);
                "UTF16LE" === b && (s = f & 255, f = s << 8 | f >> 8);
                for (m = l >>> 2; c.length <= m;) c.push(0);
                c[m] |= f << 16 - l % 4 * 8;
                l += 2
            }
    ...
    function y(a) {
        ...
        for (f = 0; f < c; f += 2) {
            ...
            if (isNaN(g)) throw "String of HEX type contains invalid characters";
            for (l = f >>> 3; b.length <= l;) b.push(0);
            b[f >>> 3] |= g << 24 - f % 8 * 4
        }
        return {
            value: b,
    ...
    function B(a) {
        let b = [],
            c, f, g;
        for (f = 0; f < a.length; f += 1) c = a.charCodeAt(f), g = f >>> 2, b.length <= g && b.push(0), b[g] |= c << 24 - f % 4 * 8;
        return {
            value: b,
            binLen: 8 * a.length
        }
    ...
    function A(a) {
        ...
        for (g = 0; g < a.length; g += 4) {
            m = a.substr(g, 4);
            for (l = h = 0; l < m.length; l += 1) f = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/".indexOf(m[l]), h |= f << 18 - 6 * l;
            for (l = 0; l < m.length - 1; l += 1) {
                for (f = c >>> 2; b.length <= f;) b.push(0);
                b[f] |= (h >>> 16 - 8 * l & 255) << 24 - c % 4 * 8;
                c += 1
            }
        }
        return {
    ...
    function D(a, b) {
        let c = "",
            f = 4 * a.length,
            g, l;
        for (g = 0; g < f; g += 1) l = a[g >>> 2] >>> 8 * (3 - g % 4), c += "0123456789abcdef".charAt(l >>> 4 & 15) + "0123456789abcdef".charAt(l & 15);
    ...
    function E(a, b) {
        let c = "",
            ...
            g, l, h;
        for (g = 0; g < f; g += 3)
            for (h = g + 1 >>> 2, l = a.length <= h ? 0 : a[h], h = g + 2 >>> 2, h = a.length <= h ? 0 : a[h], h = (a[g >>> 2] >>> 8 * (3 - g % 4) & 255) << 16 | (l >>> 8 * (3 - (g + 1) % 4) & 255) << 8 | h >>> 8 * (3 - (g + 2) % 4) & 255, l = 0; 4 > l; l += 1) c = 8 * g + 6 * l <= 32 * a.length ? c + "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/".charAt(h >>> 6 * (3 - l) & 63) : c + b.b64Pad;
        return c
    }
    ...
    function F(a) {
        let b = "",
            c = 4 * a.length,
            f, g;
        for (f = 0; f < c; f += 1) g = a[f >>> 2] >>> 8 * (3 - f % 4) & 255, b += String.fromCharCode(g);
        return b
    }
    ...
    function q(a, b) {
        let c = null;
        c = new m(a.a, a.b);
        return c = 32 >= b ? new m(c.a >>> b | c.b << 32 - b & 4294967295, c.b >>> b | c.a << 32 - b & 4294967295) : new m(c.b >>> b - 32 | c.a << 64 - b & 4294967295, c.a >>> b - 32 | c.b << 64 - b & 4294967295)
    }

    function I(a, b) {
        let c = null;
        return c = 32 >= b ? new m(a.a >>> b, a.b >>> b | a.a << 32 - b & 4294967295) : new m(0, a.a >>> b - 32)
    }

    function K(a, b, c) {
        return new m(a.a & b.a ^ ~a.a & c.a, a.b & b.b ^ ~a.b & c.b)
    }

    function L(a, b, c) {
        return new m(a.a & b.a ^ a.a & c.a ^ b.a & c.a, a.b & b.b ^ a.b & c.b ^ b.b & c.b)
    }
    ...
    function M(a) {
        let b = q(a, 28),
            c = q(a, 34);
        a = q(a, 39);
        return new m(b.a ^ c.a ^ a.a, b.b ^ c.b ^ a.b)
    }
    ...
    function Q(a, b) {
        ...
        f = (a.b >>> 16) + (b.b >>> 16) + (c >>> 16);
        g = (f & 65535) << 16 | c & 65535;
        c = (a.a & 65535) + (b.a & 65535) + (f >>> 16);
        f = (a.a >>> 16) + (b.a >>> 16) + (c >>> 16);
        return new m((f & 65535) << 16 | c & 65535, g)
    }

    function R(a, b, c, f) {
        let g, l, h;
        g = (a.b & 65535) + (b.b & 65535) + (c.b & 65535) + (f.b & 65535);
        l = (a.b >>> 16) + (b.b >>> 16) + (c.b >>> 16) + (f.b >>> 16) + (g >>> 16);
        h = (l &
            65535) << 16 | g & 65535;
        g = (a.a & 65535) + (b.a & 65535) + (c.a & 65535) + (f.a & 65535) + (l >>> 16);
        l = (a.a >>> 16) + (b.a >>> 16) + (c.a >>> 16) + (f.a >>> 16) + (g >>> 16);
        return new m((l & 65535) << 16 | g & 65535, h)
    }

    function S(a, b, c, f, g) {
        let l, h, s;
        l = (a.b & 65535) + (b.b & 65535) + (c.b & 65535) + (f.b & 65535) + (g.b & 65535);
        h = (a.b >>> 16) + (b.b >>> 16) + (c.b >>> 16) + (f.b >>> 16) + (g.b >>> 16) + (l >>> 16);
        s = (h & 65535) << 16 | l & 65535;
        l = (a.a & 65535) + (b.a & 65535) + (c.a & 65535) + (f.a & 65535) + (g.a & 65535) + (h >>> 16);
        h = (a.a >>> 16) + (b.a >>> 16) + (c.a >>> 16) + (f.a >>> 16) + (g.a >>> 16) + (l >>> 16);
        return new m((h &
            65535) << 16 | l & 65535, s)
    }

    function u(a, b, c) {
        let f, g, l, h, s, q, u, C, v, d, n, p, r, w, H, t, y, z, A, B, D, E, F, G, e, x = [],
            I, k = [1116352408, 1899447441, 3049323471, 3921009573, 961987163, 1508970993, 2453635748, 2870763221, 3624381080, 310598401, 607225278, 1426881987, 1925078388, 2162078206, 2614888103, 3248222580, 3835390401, 4022224774, 264347078, 604807628, 770255983, 1249150122, 1555081692, 1996064986, 2554220882, 2821834349, 2952996808, 3210313671, 3336571891, 3584528711, 113926993, 338241895, 666307205, 773529912, 1294757372, 1396182291, 1695183700,
        ...
        for (p = 0; p < I; p += w) {
            ...
            q = d[6];
            u = d[7];
            for (r = 0; r < n; r += 1) 16 > r ? (v = r * H + p, C = a.length <= v ? 0 : a[v], v = a.length <= v + 1 ? 0 : a[v + 1], x[r] = new e(C, v)) : x[r] = y(B(x[r - 2]), x[r - 7], A(x[r - 15]), x[r - 16]), C = z(u, E(h), F(h, s, q), k[r], x[r]), v = t(D(b), G(b, f, g)), u = q, q = s, s = h, h = t(l, C), l = g, g = f, f = b, b = t(C, v);
            d[0] = t(b, d[0]);
            d[1] = t(f, d[1]);
            d[2] = t(g, d[2]);
            d[3] = t(l, d[3]);
        ...
        if ("SHA-384" === c) a = [d[0].a, d[0].b, d[1].a, d[1].b, d[2].a, d[2].b,
            d[3].a, d[3].b, d[4].a, d[4].b, d[5].a, d[5].b
        ];
        else if ("SHA-512" === c) a = [d[0].a, d[0].b, d[1].a, d[1].b, d[2].a, d[2].b, d[3].a, d[3].b, d[4].a, d[4].b, d[5].a, d[5].b, d[6].a, d[6].b, d[7].a, d[7].b];
        else throw "Unexpected error in SHA-2 implementation";
        return a
    }
